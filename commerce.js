...
(() => {
  ...
  function ownerTable() {
    const names = { pilot: tr("Piloto", "Pilot"), contact_requested: tr("Contacto pendiente", "Contact requested"), archived: tr("Archivada", "Archived") };
    const rows = ownerRows.map(row => `<tr><td>${escape(row.name)}<small>${escape(row.ownerEmail)}</small></td><td><select data-pilot-status="${escape(row.id)}">${Object.entries(names).map(([value,label]) => `<option value="${value}" ${row.status === value ? "selected" : ""}>${label}</option>`).join("")}</select></td><td><input type="date" aria-label="${tr("Fin del piloto", "Pilot end date")}" data-pilot-date="${escape(row.id)}" value="${escape(row.pilotEndsAt || "")}"></td><td><button class="btn light" data-pilot-save="${escape(row.id)}">${tr("Guardar", "Save")}</button></td></tr>`).join("");
    return `<p>${tr("Directorio comercial. No contiene expedientes clínicos. El estado y la fecha del piloto son administrativos; no bloquean el acceso ni eliminan datos.", "Business directory only. No patient records are shown. Pilot status and dates are administrative tracking fields; they do not block access or delete data.")}</p><p>${tr("Aparecen las clínicas que han sincronizado su perfil al iniciar sesión como administrador.", "Clinics appear after an administrator signs in and their business profile is synced.")}</p><strong>${tr("Clínicas cargadas", "Clinics loaded")}: ${ownerRows.length} · ${tr("En piloto", "In pilot")}: ${ownerRows.filter(row => row.status === "pilot").length}</strong><div class="platform-table-wrap"><table><thead><tr><th>${tr("Clínica", "Clinic")}</th><th>${tr("Estado", "Status")}</th><th>${tr("Fin del piloto", "Pilot end date")}</th><th></th></tr></thead><tbody>${rows || `<tr><td colspan="4">${tr("Todavía no hay clínicas sincronizadas.", "No clinics have synced their profiles yet.")}</td></tr>`}</tbody></table></div>${nextCursor ? `<button class="btn light" data-owner-more>${tr("Cargar más", "Load more")}</button>` : ""}<h3>${tr("Servicios pendientes de configuración", "Services awaiting setup")}</h3><p>${tr("Suscripciones, soporte, alertas y supervisión de respaldos todavía no están conectados. No se muestran indicadores ficticios de funcionamiento.", "Subscriptions, support, alerts, and backup monitoring are not connected yet. No simulated operational status is displayed.")}</p>`;
  }
  async function owner(loadMore = false) {
    ...
    try {
      ...
      shell(tr("Administración de plataforma", "Platform administration"), ownerTable());
  ...
  async function refreshAccess(user) {
    ...
    if (!user) { ownerRows = []; nextCursor = null; if (dialog.open) dialog.close(); return; }
    ...
    if (currentAccess.role === "admin") api("syncClinic").catch(() => {});
  ...
  function installMenu() {
    ...
    group.innerHTML = `<button class="btn light" data-platform="subscription">${tr("Suscripción", "Subscription")}</button><button class="btn light" data-platform="owner" data-owner-only hidden>${tr("Mi plataforma", "My platform")}</button>`;
    nav.after(group);
  ...
  document.addEventListener("click", async event => {
    ...
    if (target.dataset.platform) target.dataset.platform === "owner" ? owner() : subscription();
    if (target.hasAttribute("data-platform-close")) dialog.close();
    ...
    if (target.dataset.pilotSave && !busy) {
      busy = true; target.disabled = true;
      const id = target.dataset.pilotSave;
      const row = ownerRows.find(item => item.id === id);
      const container = target.closest("tr");
      try {
        const status = container.querySelector("select").value;
        const pilotEndsAt = container.querySelector("input").value || null;
        await api("updatePilot", { clinicId: id, status, pilotEndsAt });
        Object.assign(row, { status, pilotEndsAt });
        notice(tr("Cambios guardados y registrados en la auditoría.", "Changes saved and recorded in the audit log."));
      } catch (error) { notice(errorMessage(error)); }
      finally { busy = false; target.disabled = false; }
    }
  });
  window.addEventListener("clinic-language-change", () => {
    ...
    installMenu();
    document.querySelectorAll("[data-owner-only]").forEach(node => node.hidden = !ownerAccess);
    if (dialog.open) dialog.dataset.view === "owner" ? owner() : subscription();
  });
