...
function say(message) { const notice = $("#notice"); notice.textContent = T(message); notice.classList.add("show"); setTimeout(() => notice.classList.remove("show"), 3000); }
function text(es, en) { return ClinicI18n.language === "en" ? en : es; }
...
function applySession(data) { if (["en", "es"].includes(data.language)) ClinicI18n.setLanguage(data.language); session = data; }
async function activate(event) { event.preventDefault(); code = $("#accessCode").value.trim().toUpperCase(); try { applySession(await api("get")); sessionStorage.setItem("roomPortalCode", code); render(); } catch (error) { say(error.message); } }
async function submitForm(event) { event.preventDefault(); const item = session.activities[session.currentIndex]; const answers = Object.fromEntries(Array.from(document.querySelectorAll("[data-answer]")).map((input) => [input.dataset.answer, input.value.trim()])); try { await api("submitForm", { responseId: item.responseId, answers }); session.currentIndex += 1; render(); } catch (error) { say(error.message); } }
...
document.querySelectorAll("[data-language]").forEach(button => button.addEventListener("click", () => ClinicI18n.setLanguage(button.dataset.language)));
window.addEventListener("clinic-language-change", () => {
  if (!session || requestPending) return;
  if ((session.currentIndex || 0) >= session.activities.length) {
    const done = $("#content .done");
    if (done) done.innerHTML = `<span>✓</span><h1>${text("¡Gracias!", "Thank you!")}</h1><p>${text("Todo quedó guardado. Entrega el iPad al personal.", "Your responses have been saved. Please return the iPad to staff.")}</p>`;
    return;
  }
  const answers = [...document.querySelectorAll("[data-answer]")].map(node => [node.dataset.answer,node.value]);
  const signer = $("#signer")?.value;
  const consent = $("#consent")?.checked;
  const oldCanvas = $("#canvas");
  const signature = oldCanvas?.dataset.ink === "true" ? oldCanvas.toDataURL() : null;
  render();
  for (const [id,value] of answers) { const node = [...document.querySelectorAll("[data-answer]")].find(input => input.dataset.answer === id); if (node) node.value = value; }
  if ($("#signer") && signer !== undefined) $("#signer").value = signer;
  if ($("#consent")) $("#consent").checked = Boolean(consent);
  if (signature && $("#canvas")) { const canvas = $("#canvas"); const img = new Image(); img.onload = () => { const ctx = canvas.getContext("2d"); ctx.save(); ctx.setTransform(1,0,0,1,0,0); ctx.drawImage(img,0,0,canvas.width,canvas.height); ctx.restore(); canvas.dataset.ink = "true"; }; img.src = signature; }
});
if (startupCode) { code = startupCode; sessionStorage.setItem("roomPortalCode", code); api("get").then((data) => { applySession(data); render(); }).catch((error) => { lock(); say(error.message); }); }
